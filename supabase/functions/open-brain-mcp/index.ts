...
server.registerTool(
  "update_thought",
  {
    ...
    description: "Update an existing thought's content. Re-generates embedding and metadata automatically.",
    inputSchema: {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          content,
          embedding,
          metadata: { ...metadata, source: "mcp", updated_via: "update_thought" },
...
server.registerTool(
  "delete_thought",
  {
    ...
    description: "Permanently delete a thought by its ID.",
    inputSchema: {
  ...
  async ({ thought_id }) => {
    try {
      // Verify thought exists before deleting
      const { data: existing, error: findErr } = await supabase
        .from("thoughts")
        .select("id")
        .eq("id", thought_id)
